...
class GoogleMapsQueryService{
    ...
    func loadPOIs(query: GoogleCategory, latitude: Double, longitude: Double) -> Promise<GoogleMapsPlaceMarkService>? {
             
        var urlString = ""
        switch query {
        case .bus_station:
            urlString = "bus_station"
        case .cafe:
            urlString = "cafe"
        case .car_repair:
            urlString = "car_repair"
        case .library:
            urlString = "library"
        case .restaurant:
    ...
    //        guard let url = URL(string:"https://maps.googleapis.com/maps/api/place/textsearch/json?query=" + urlString + "&location="+String(longitude)+","+String(latitude)+"&radius=10000&key=AIzaSyCvuaYDzCqq7GvOvey6pZSn4bhY92iq7E8")
            ...
            guard let url = URL(string:"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+String(latitude)+","+String(longitude)+"&radius=10000&type=" + urlString + "&key=AIzaSyDzZ2ixVoy_vDGRrc52Axw45YifTu0qvQQ")
            else {
                    print("Invalid URL")
                    return nil
                }
...
//        guard let url = URL(string:"https://maps.googleapis.com/maps/api/place/textsearch/json?query=" + urlString + "&location="+String(longitude)+","+String(latitude)+"&radius=10000&key=AIzaSyCvuaYDzCqq7GvOvey6pZSn4bhY92iq7E8")
        ...
        guard let url = URL(string:"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+String(latitude)+","+String(longitude)+"&radius=10000&keyword=" + urlString + "&key=AIzaSyDzZ2ixVoy_vDGRrc52Axw45YifTu0qvQQ")
        else {
                print("Invalid URL")
                return nil
            }
    ...
    func setPlaceMark(GoogleMapsPlaceMark: GoogleMapsPlaceMark) -> [PlaceMarkForAllMap]{
        ...
        for i in GoogleMapsPlaceMark.name.indices {
            ...
            temp.placeName = GoogleMapsPlaceMark.vicinity[i]
            temp.latitude = GoogleMapsPlaceMark.lat[i]
            temp.longitude = GoogleMapsPlaceMark.lng[i]
            temp.place_id = GoogleMapsPlaceMark.place_id[i]
...
extension GoogleMapsQueryService: QueryServiceAccess{
    ...
    func fetchPOIs(query: GoogleCategory, latitude: Double, longitude: Double) -> [PlaceMarkForAllMap]?{
        ...
        let promise =
            firstly{
                loadPOIs(query: query, latitude: latitude, longitude: longitude)!
            }
            .done() { placeMarkStores in
